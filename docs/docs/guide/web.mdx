...
- Web 登录先取 `GET /api/auth/sm2/public-key`,校验公钥指纹后以 SM2 C1C3C2 Base64 密文提交 `POST /api/auth/login`:`{username,password,password_encoding:"sm2-c1c3c2-base64"}`。服务端默认返回完整 64 字符 SHA-256 指纹,客户端兼容旧 16 字符前缀。生产构建必须配置匹配的 `VITE_SM2_PUBLIC_KEY_FINGERPRINT`;登录请求失败会丢弃缓存公钥并重新获取。
- 后端只接受 `password_encoding:"sm2-c1c3c2-base64"` 的登录请求，未带或不支持该字段的请求会被拒绝。SM2 是应用层传输加密，服务端仍会在校验密码前解密；HTTPS/TLS 仍是部署前提。
...
- **`Dockerfile`**(生产,多阶段):`node:24-alpine` 跑 `npm run build`,`nginx:stable-alpine` 运行;`:80`;`/api/` 代理到 `http://backend:8080`,`location /` 用 `try_files $uri $uri/ /index.html` 做 SPA fallback。构建期注入 `VITE_ICP_NUMBER`/`VITE_DOCS_URL`/`VITE_SM2_PUBLIC_KEY_FINGERPRINT`。
...
| `VITE_SM2_PUBLIC_KEY_FINGERPRINT` | `import.meta.env`(构建期内联) | Web 登录公钥 pin;推荐完整 64 位 SHA-256,兼容 16 位前缀 | 生产必填 |
