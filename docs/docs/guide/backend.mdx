...
| `SM2_PRIVATE_KEY` / `SM2_PUBLIC_KEY` | 无 | Web 登录 SM2 密钥对;后端启动必填 |
| `SM2_PUBLIC_KEY_FINGERPRINT` | 空 | Web 构建期公钥 pin;推荐完整 64 位 SHA-256,兼容 16 位前缀 |
...
- compose:`docker-compose.yml`(dev,backend 用 `Dockerfile.dev` + `command: air`,`APP_ENV=development`)、`docker-compose.prod.yml`(prod,backend 用 `Dockerfile`,对数据库、JWT、管理员密码和 SM2 密钥用 `${VAR:?...}` fail-fast)。
