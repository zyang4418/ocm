...
- **observation 文档后端（Renderer）注册**：下游在自有包 `init()` 调 `observation.RegisterRendererFactory(func(db) (Renderer, error))`（`backend/internal/observation/renderer_registry.go`），`main.go` 接线听课评课时经 `observation.ResolveRenderer` 构建注入；未注册即开源默认（CRUD/提交可用，模板/导出端点报 `ErrRendererMissing`）。时序：ResolveRenderer 在内置迁移之后、**modules 迁移循环之前**执行——工厂需要自己的表就自行迁移（`CREATE TABLE IF NOT EXISTS` 惯例）。
