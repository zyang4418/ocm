...
// — deploy/mosquitto/auth-init.sh seeds the backend user and one user per
// declared source via password_file + acl_file — so nothing implements this
// yet. The classroom controller runtime (per-device credentials issued at
// claim time, revoked at unbind) is the expected first implementation; keeping
// the interface here pins the shape the bootstrap must eventually match.
